...
public sealed class Product : BaseEntity
{
    ...
    /// <summary>External source URL when product is imported.</summary>
    public string? SourceUrl { get; private set; }

    /// <summary>UTC timestamp when product was imported from external source.</summary>
    ...
    {
        var slug = GenerateSlug(nameEn, nameAr, sku);
    ...
    private static string GenerateSlug(string nameEn, string nameAr, string sku)
    {
        var fromEn = SlugifyCore(nameEn);
        if (!string.IsNullOrWhiteSpace(fromEn))
        {
            return fromEn;
        }

        var fromAr = SlugifyCore(nameAr);
        if (!string.IsNullOrWhiteSpace(fromAr))
        {
            return fromAr;
        }

        var fromSku = SlugifyCore(sku);
        return !string.IsNullOrWhiteSpace(fromSku)
            ? $"product-{fromSku}"
            : $"product-{Guid.NewGuid():N}";
    }

    private static string SlugifyCore(string? value)
    {
        ...
        // Keep letters/numbers from any language and collapse other separators into '-'.
        var slug = Regex.Replace(normalized, @"[^\p{L}\p{Nd}]+", "-");
    ...
    /// <summary>
    /// Deducts stock for an order. Raises LowStockEvent if at or below threshold (2).
    /// </summary>
    ...
    public void DeductStock(int quantity)
    {
        ...
        // Raise low stock alert if at or below threshold (2)
        if (StockQuantity <= 2)
        {
    ...
    /// <summary>Public wrapper for DeductStock to reduce inventory when order is placed.</summary>
        ...
        string? descriptionAr, string? descriptionEn, int categoryId)
    {
        NameAr = nameAr;
        NameEn = nameEn;
        Slug = GenerateSlug(nameEn, nameAr, SKU);
        DescriptionAr = descriptionAr;
        DescriptionEn = descriptionEn;
        CategoryId = categoryId;
    }

    /// <summary>Sets import metadata for products created through importer flow.</summary>
    public void SetImportMetadata(string? sourceUrl, DateTime? importedAt)
    {
